...
# binarize image
thresh = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, BLOCK_SIZE, C)

# morphology just made results worse, not used
# kernel = np.ones((3, 3), np.uint8)
# thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, kernel, iterations=1)
# thresh = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel, iterations=1)

# test binarization combos (not used, switched to adaptive binary)
# for t in range(1, 11):
#     _, thresh = cv2.threshold(blurred, 10 * t, 255, cv2.THRESH_BINARY_INV)
#     cv2.imshow(f'{t=}', thresh)
...
# print defect count
print(f'defect count = {len(centroids)}')
